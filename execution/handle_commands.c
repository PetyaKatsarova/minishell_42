...
/*   Updated: 2025/04/18 16:58:06 by pekatsar      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int	handle_commands(t_env_list *env_struct, t_node *cmd_node)
{
	...
	//	return do_exit(input, env_struct, cmd_node);
	//else 
	if (cmd_node->token_type == TOKEN_PWD)
		exit_status = get_pwd();
	...
	{
		//if (exec_on_path(env_struct, cmd_node->argv, tree->num_pipes) == EXIT_FAILURE)
		//	return (EXIT_FAILURE);
	}
